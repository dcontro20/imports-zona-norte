  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 20 }}>
        ...
        <StatCard label="Ventas mes" value={salesMonth.length} sub={`${formatMoney(revenueMonth)} ARS`} icon="📈" color="#6366f1" />
      ...
      <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
        <Card style={{ flex: 1, minWidth: 280 }}>
          <h4 style={{ color: "#6366f1", margin: "0 0 14px", fontSize: 14, textTransform: "uppercase", letterSpacing: 0.5 }}>🏆 Top 5 más vendidos</h4>
          {topProducts.length === 0 ? <p style={{ color: "#9ca3af", fontSize: 13 }}>Sin ventas aún</p> :
            topProducts.map((p, i) => (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #edf0f2" }}>
                <span style={{ color: "#4b5563", fontSize: 13 }}>{i + 1}. {p.name}</span>
        ...
        <Card style={{ flex: 1, minWidth: 280 }}>
          ...
          {lowStock.length === 0 ? <p style={{ color: "#9ca3af", fontSize: 13 }}>Todo bien por ahora</p> :
            lowStock.slice(0, 8).map(p => (
              <div key={p.id} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #edf0f2" }}>
                <span style={{ color: "#4b5563", fontSize: 13 }}>{p.brand} {p.model} - {p.flavor}</span>
