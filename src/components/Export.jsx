...
export const ExportData = ({ products, sales, purchases, expenses, withdrawals, cashMovements, stockLog, exchangeRate }) => {
  ...
  const exports = [
    { label: "Stock actual", sub: `${products.length} productos`, icon: "📦", color: "#a855f7", fn: exportStock },
    { label: "Ventas", sub: `${sales.length} registros`, icon: "🛒", color: "#00b894", fn: exportSales },
    { label: "Compras", sub: `${purchases.length} pedidos`, icon: "🚚", color: "#6c5ce7", fn: exportPurchases },
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <div>
          <h2 style={{ color: "#e0e0ff", margin: 0, fontSize: 22 }}>Exportar Datos</h2>
          <span style={{ color: "#6666aa", fontSize: 13 }}>Descargá toda la información del sistema en CSV (compatible con Excel y Google Sheets)</span>
        </div>
        <Btn onClick={exportAll} style={{ background: exporting ? "#555" : "linear-gradient(135deg, #6c5ce7, #a855f7)" }}>
          ...
          <Card key={exp.label} style={{ cursor: "pointer", transition: "all 0.2s", border: `1px solid ${exp.color}22` }}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ color: "#6666aa", fontSize: 12 }}>{exp.sub}</div>
              </div>
              <span style={{ marginLeft: "auto", color: "#6666aa", fontSize: 18 }}>⬇</span>
            </div>
          </Card>
        ))}
      </div>

      <Card style={{ marginTop: 20, background: "#0d0d1a", border: "1px solid #2a2a4a" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ fontSize: 18 }}>💡</span>
          <span style={{ color: "#6666aa", fontSize: 13 }}>
