  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <h2 style={{ color: "#1a1a2e", margin: 0, fontSize: 22 }}>Compras / Importaciones ({purchases.length})</h2>
      ...
      <Card>
        <Table columns={[
          ...
          { key: "costoTotal", label: "Costo total", render: r => {
            ...
            return hasCosts ? formatMoney(r.totalCostARS) : <span style={{ color: "#6b7280", fontSize: 11 }}>incompleto</span>;
          }},
      ...
      <Modal open={modal} onClose={() => { setModal(false); setEditing(null); }} title={editing ? "Editar Pedido" : "Nuevo Pedido"}>
        ...
        {form.groups.map((group, gi) => {
          ...
          return (
            <div key={gi} style={{ background: "#f7f8fa", border: `1px solid ${bc}33`, borderRadius: 12, padding: 14, marginBottom: 12 }}>
              ...
              {group.brand && (<>
                <label style={{ display: "block", fontSize: 11, color: "#6b7280", marginBottom: 6, fontWeight: 600, textTransform: "uppercase" }}>Sabores</label>
                ...
                {group.flavors.length > 0 && <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid #edf0f2", display: "flex", justifyContent: "space-between" }}>
                  <span style={{ color: "#6b7280", fontSize: 12 }}>{group.flavors.reduce((s, f) => s + (Number(f.qty) || 0), 0)} uds</span>
                  <span style={{ color: "#4b5563", fontSize: 12, fontWeight: 600 }}>{formatMoney(group.flavors.reduce((s, f) => s + (Number(f.qty) || 0), 0) * (Number(group.unitCostUSDT) || 0), "USDT")}</span>
                </div>}
              </>)}
            </div>
          );
        })}
        <button onClick={addGroup} style={{ background: "#fff", border: "2px dashed #e2e4e9", color: "#6366f1", padding: "12px", borderRadius: 12, cursor: "pointer", fontSize: 13, fontWeight: 600, width: "100%", marginBottom: 14 }}>+ Agregar marca/modelo al pedido</button>
        ...
        <div style={{ background: "#f7f8fa", border: "1px solid #e2e4e9", borderRadius: 10, padding: 14, marginBottom: 14 }}>
          ...
          <label style={{ display: "block", fontSize: 11, color: "#6b7280", marginBottom: 6, fontWeight: 600, textTransform: "uppercase" }}>Comisión proveedor (USDT)</label>
          ...
          <div style={{ borderTop: "1px solid #edf0f2", paddingTop: 10, marginTop: 6 }}>
            <label style={{ display: "block", fontSize: 11, color: "#6b7280", marginBottom: 6, fontWeight: 600, textTransform: "uppercase" }}>Pasero + Envío (Pesos)</label>
            <div style={{ display: "flex", gap: 12 }}>
        ...
        {/* Total */}
        {totalItems > 0 && <div style={{ background: "#f7f8fa", borderRadius: 10, padding: 14, marginBottom: 14, border: "1px solid #e2e4e9" }}>
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ color: "#6b7280", fontSize: 13 }}>Vapes ({totalItems} uds)</span>
            <span style={{ color: "#4b5563" }}>{formatMoney(productsUSDT, "USDT")}</span>
          </div>
          {supplierCommUSDT > 0 && <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
            <span style={{ color: "#6b7280", fontSize: 13 }}>Comisión proveedor</span>
            <span style={{ color: "#26de81" }}>{formatMoney(supplierCommUSDT, "USDT")}</span>
          </div>}
          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, paddingTop: supplierCommUSDT > 0 ? 4 : 0, borderTop: supplierCommUSDT > 0 ? "1px solid #edf0f2" : "none" }}>
            <span style={{ color: "#6b7280", fontSize: 13, fontWeight: supplierCommUSDT > 0 ? 600 : 400 }}>Total USDT transferido</span>
            <span style={{ color: "#4b5563", fontWeight: 600 }}>{formatMoney(totalUSDTwithComm, "USDT")} · ~{formatMoney(Math.round(totalUSDTwithComm * exchangeRate))}</span>
          </div>
          {paseroARS > 0 && <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span style={{ color: "#6b7280", fontSize: 13 }}>Pasero</span><span style={{ color: "#fdcb6e" }}>{formatMoney(paseroARS)}</span></div>}
          {envioARS > 0 && <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span style={{ color: "#6b7280", fontSize: 13 }}>Envío</span><span style={{ color: "#fdcb6e" }}>{formatMoney(envioARS)}</span></div>}
          <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid #e2e4e9", paddingTop: 8, marginTop: 4 }}>
            <span style={{ color: "#1a1a2e", fontSize: 15, fontWeight: 700 }}>Costo total</span>
            <span style={{ color: "#e74c3c", fontSize: 18, fontWeight: 800 }}>{formatMoney(totalCostARS)}</span>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
            <span style={{ color: "#6b7280", fontSize: 12 }}>Costo por unidad</span>
            <span style={{ color: "#6b7280", fontSize: 13 }}>{formatMoney(Math.round(totalCostARS / totalItems))} / ud</span>
          </div>
        </div>}
      ...
      <Modal open={!!verifyModal} onClose={() => setVerifyModal(null)} title="✅ Verificar Pedido">
        {verifyPurchase && (<div>
          <div style={{ color: "#6b7280", fontSize: 13, marginBottom: 16 }}>Pedido de <strong style={{ color: "#1a1a2e" }}>{verifyPurchase.supplier}</strong> del {formatDate(verifyPurchase.date)}</div>
          <div style={{ background: "#f7f8fa", borderRadius: 10, padding: 14, marginBottom: 16 }}>
            ...
            {(verifyPurchase.items || []).map((item, i) => {
              ...
              return (<div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #edf0f2" }}>
                <span style={{ color: "#4b5563", fontSize: 14 }}>{prod ? `${prod.brand} ${prod.model} - ${prod.flavor}` : "?"}</span>
                <Badge color="#00b894">x{item.qty}</Badge>
              </div>);
            })}
            <div style={{ display: "flex", justifyContent: "space-between", paddingTop: 10 }}>
              <span style={{ color: "#1a1a2e", fontWeight: 700 }}>Total</span>
