  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <h2 style={{ color: "#e0e0ff", margin: 0, fontSize: 22 }}>Precios</h2>
        {editMode ? (
      ...
      <Card style={{ marginBottom: 14 }}>
        <h4 style={{ color: "#a855f7", margin: "0 0 14px", fontSize: 14, textTransform: "uppercase" }}>Precios por modelo</h4>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                {["Marca", "Modelo", "Puffs", "Precio USD", "Precio ARS", "Sabores"].map(h => (
                  <th key={h} style={{ textAlign: "left", padding: "10px 12px", fontSize: 11, color: "#6666aa", textTransform: "uppercase", borderBottom: "1px solid #2a2a4a", fontWeight: 700 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {modelPrices.map(m => {
                ...
                return (
                  <tr key={m.key} style={{ background: changed ? "#a855f711" : "transparent" }}>
                    <td style={{ padding: "10px 12px", fontSize: 13, color: "#c0c0e0", borderBottom: "1px solid #1a1a30" }}>
                      <Badge color={BRAND_COLORS[m.brand] || "#a855f7"}>{m.brand}</Badge>
                    </td>
                    <td style={{ padding: "10px 12px", fontSize: 13, color: "#e0e0ff", borderBottom: "1px solid #1a1a30", fontWeight: 600 }}>{m.model}</td>
                    <td style={{ padding: "10px 12px", fontSize: 13, color: "#8888aa", borderBottom: "1px solid #1a1a30" }}>{Number(m.puffs).toLocaleString("es-AR")}</td>
                    <td style={{ padding: "10px 12px", borderBottom: "1px solid #1a1a30" }}>
                      {editMode ? (
                        <input type="number" value={newPrices[m.key] ?? m.priceUSD}
                          ...
                          style={{
                            ...
                            background: changed ? "#a855f722" : "#12122a",
                            border: `1px solid ${changed ? "#a855f7" : "#2a2a4a"}`,
                    ...
                    <td style={{ padding: "10px 12px", fontSize: 14, color: changed ? "#a855f7" : "#c0c0e0", borderBottom: "1px solid #1a1a30", fontWeight: 600 }}>
                      {formatMoney(arsPrice)}
                    </td>
                    <td style={{ padding: "10px 12px", fontSize: 13, color: "#6666aa", borderBottom: "1px solid #1a1a30" }}>
                      {m.inStock}/{m.count}
                    </td>
                  </tr>
                );
      ...
      <Card>
        ...
        {(!priceLog || priceLog.length === 0) ? (
          <div style={{ textAlign: "center", padding: 20 }}>
            <span style={{ color: "#555", fontSize: 13 }}>Todavía no hay cambios de precio registrados. Cuando edites precios, va a aparecer acá.</span>
          </div>
        ) : (
          <Table columns={[
