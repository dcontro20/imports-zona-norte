...
export const Withdrawals = ({ withdrawals, setWithdrawals, products, setProducts, logStock, currentUser }) => {
  ...
  const save = () => {
    ...
    const withdrawal = { ...form, id: uid(), qty: Number(form.qty), costEstimateUSD: costEstimate * Number(form.qty), withdrawType: form.withdrawType, createdBy: currentUser?.name || "" };
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
        <h2 style={{ color: "#e0e0ff", margin: 0, fontSize: 22 }}>Mermas (Consumo, Garantías, Canjes)</h2>
        <Btn onClick={() => setModal(true)}>+ Registrar Merma</Btn>
      </div>

      <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginBottom: 20 }}>
        <StatCard label="Diego" value={`${totalMine} uds`} icon="🚬" color="#a855f7" />
