...
describe("listaEscalonesText — el mensaje compartible", () => {
  // Se mandan los DOS mensajes, en este orden: primero el stock (lleva las
  // reglas), después el catálogo (continuación, no las repite).
  ...
  it("manda TODOS los escalones por modelo, no uno solo", () => {
    ...
    // El símbolo de moneda va UNA vez, arriba — no cuatro por renglón. Las
    // reglas viajan en la lista de STOCK, que es la que se manda primero.
    expect(stk).toContain("*Precios en pesos* — unidades: 20-49 / 50-99 / 100-199 / 200+");
  ...
  it("la mezcla libre está ESCRITA en el mensaje, no implícita (y una sola vez)", () => {
    expect(stk).toContain("El precio por unidad depende del TOTAL de unidades del pedido.");
    expect(stk).toContain("mezclá modelos y sabores como quieras");
    // Vive junto al mínimo, en positivo — no repetida arriba y abajo.
    expect(stk.toLowerCase().split("mezclá modelos y sabores").length - 1).toBe(1);
  });
  ...
  it("el mínimo va EN POSITIVO y solo en unidades — el ticket en pesos no se comunica", () => {
    expect(stk).toContain("📦 Comprás desde 20 unidades — mezclá modelos y sabores como quieras.");
    expect(stk).not.toContain("Pedido mínimo");
    expect(stk).not.toContain("ticket");
    expect(stk).not.toContain("$206.000"); // el ticket mínimo (200 USD × 1030) no aparece
  });
  ...
  it("versión en USD: precios en dólares y la conversión se explica, sin necesitar FX", () => {
    ...
    expect(usdStock).toContain("unidades: 20-49 / 50-99 / 100-199 / 200+");
  });
  ...
  it("cuál es la resumida se puede invertir sin tocar el modo (por si cambia el orden de envío)", () => {
    const stockResumido = listaEscalonesText(LISTA, { ...OPTS, modo: "stock", resumido: true });
    const catalogoCompleto = listaEscalonesText(LISTA, { ...OPTS, modo: "catalogo", resumido: false });
    expect(catalogoCompleto).toContain("Comprás desde 20 unidades");
    expect(catalogoCompleto).toContain("unidades: 20-49");
    expect(stockResumido).not.toContain("Comprás desde 20 unidades");
    // El modo sigue mandando QUÉ se ofrece; resumido solo decide las reglas.
    expect(stockResumido).not.toContain("V250");
    expect(catalogoCompleto).toContain("V250");
  });
  ...
  it("no asume 4 escalones: una política de 2 produce 2 columnas", () => {
    ...
    const t = listaEscalonesText(lista2, { ...OPTS, modo: "stock" });
